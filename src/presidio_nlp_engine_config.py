...
def create_nlp_engine_with_flair(
    ...
    """
    from src.flair_recognizer import FlairRecognizer
...
def create_nlp_engine_with_azure_ai_language(ta_key: str, ta_endpoint: str):
    ...
    """
    from src.azure_ai_language_wrapper import AzureAIServiceWrapper
...
def create_nlp_engine_with_llm(model):
    from src.llm_recognizer import LlmRecognizer

    registry = RecognizerRegistry()
    registry.load_predefined_recognizers()

    llm_recognizer = LlmRecognizer(model_name=model)
